...
for file_name in [e for e in glob.glob('HDF5_*') if not e.endswith('.tif')]:
    ...
    fullCmd = 'gdal_translate -a_srs  "+proj=geos +a=6378169 +b=6356583.8 +lon_0=0 +h=35785831" -a_ullr -5570248.832537 5570248.832537 5570248.832537 -5570248.832537 HDF5:"%s"://DSSF %s' %(os.path.join(input_folder_landsaf, file_name_only), output_GOES_projected)
    process = subprocess.Popen(fullCmd)
    process.wait() 
